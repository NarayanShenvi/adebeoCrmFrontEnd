...
export const updateProductCategoryAsync = createAsyncThunk(
  ...
  async ({ categoryCode, updateData }, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem('Access_Token');
      const response = await axios.put(
        `${API}/updatecategory/${categoryCode}`,
        updateData,
        { headers: token ? { Authorization: `Bearer ${token}` } : {} }
      );
...
const productCategorySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateProductCategoryAsync.fulfilled, (state, action) => {
        state.loading = false;
        state.successMessage = action.payload.message || "Category updated successfully";
      })
      .addCase(updateProductCategoryAsync.rejected, (state, action) => {
        state.loading = false;
