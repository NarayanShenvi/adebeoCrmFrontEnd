  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCustomerPaymentsAsync.fulfilled, (state, action) => {
        ...
        state.payments = payments || [];  // Default to empty array if undefined
        state.totalCount = totalCount || 0;  // Default to 0 if undefined
        state.totalPages = totalPages || 1;  // Default to 1 if undefined
        state.currentPage = currentPage || 1;  // Default to 1 if undefined
      });
    ...
    builder
      ...
      .addCase(processCustomerPaymentAsync.fulfilled, (state, action) => {
        ...
        // Find the updated payment in the payments list and update it
        state.payments = state.payments.map(payment => 
          payment.invoice_number === updatedPayment.invoice_number 
            ? { 
                ...payment, 
                ...updatedPayment,  // Merge the updated data
                amount_due: payment.amount_due - updatedPayment.paid_amount,  // Update the amount_due after payment
            }
            : payment
        );
      })
